...
//  virtualReality
//
//  Created by Abhi Gupta on 2016-04-25.
//  Copyright © 2016 Abhi Gupta. All rights reserved.
//
...
//#include <GLUT/glut.h> // can't include glut.h and freeglut.cch both because of conflicting method signatures
...
void setGlCamera()
{
    if(bProjectionMode)
    {
        ...
        float pb[3]={cx,-cy,0};
        float pc[3]={-cx,cy,0};
        float pe[3]={static_cast<float>(glCamX),static_cast<float>(glCamY),static_cast<float>(glCamZ)};
        // space points
        cv::Vec3f Pa(pa);
        cv::Vec3f Pb(pb);
        cv::Vec3f Pc(pc);
